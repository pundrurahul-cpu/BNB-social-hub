...
async function planMonthlyStrategy(clientId, month, year) {
  console.log(`🧠 [Strategy Engine] Building plan for ${clientId} (${month}/${year})...`);

  try {
    // 1. Get Client DNA (Fixed Days, Voice, focus)
    ...
    // 2. Identify Dates (Mon, Wed, Fri + Occasions)
    const plannedSlots = [];
    ...
    // 3. Brainstorm Content for each slot (Ensuring Uniqueness)
    ...
    console.error('❌ Strategy Engine Failed:', err.message);
    throw err;
  }
}
...
async function generateStrategicBrief(strategy, blueprint, context) {
  ...
  const prompt = `
    ...
    Return ONLY JSON:
    {
      ...
      "copy_direction": "Brief for the writer/designer on the core message",

...
async function generateStrategicMonthlyPlan(clientId, month, year) {
  ...
  try {
    ...
    for (let i = 0; i < plannedSlots.length; i++) {
      ...
      // 4. Brainstorm via AI (Gemini + FastAPI check)
      ...
      const brief = await generateStrategicBrief(strategy, blueprint, slot.reason, pastTopics);
      ...
      await supabase.from('posts').insert([{
        ...
        metadata: {
          visual_prompt: brief.visual_idea, // Description for designer
          automation_version: 'StrategicPlanner_v7'
        }
      }]);
    }
...
async function generateStrategicBrief(strategy, blueprint, context, history) {
  ...
  const prompt = `
    ...
    Output ONLY valid JSON:
    {
      ...
      "visual_idea": "A detailed visual brief for a designer to create a referral moodboard",

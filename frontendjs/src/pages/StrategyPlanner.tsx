...
export function StrategyPlanner({ onNavigate }: { onNavigate: (path: string, params?: any) => void }) {
  ...
  const fetchPlan = async () => {
    setLoading(true);
    try {
      const response = await fetch(`http://103.155.85.64:5001/api/posts?client_id=${activeClient.id}`);
      if (response.ok) {
  ...
  const handleGenerateVisual = async (postId: string) => {
    ...
    try {
      const response = await fetch(`http://103.155.85.64:5001/api/posts/${postId}/generate-visual`, {
        method: 'POST'
      });
      if (response.ok) {
        await fetchPlan();
  ...
  const handleGenerateQuote = async (postId: string) => {
    ...
    try {
      const response = await fetch(`http://103.155.85.64:5001/api/posts/${postId}/generate-quote-graphic`, {
        method: 'POST'
      });
      if (response.ok) {
        await fetchPlan();

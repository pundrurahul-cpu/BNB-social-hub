...
export function StrategyPlanner({ onNavigate }: { onNavigate: (path: string, params?: any) => void }) {
  ...
  const fetchPlan = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/posts?client_id=${activeClient.id}`);
      if (response.ok) {
  ...
  const handleGenerateStrategy = async () => {
    ...
    try {
      const now = new Date();
      const response = await fetch('/api/automation/plan-month', {
        method: 'POST',
  ...
  const handleGenerateVisual = async (postId: string) => {
    ...
    try {
      const response = await fetch(`/api/posts/${postId}/generate-visual`, {
        method: 'POST'
      });
      if (response.ok) {
        await fetchPlan();
  ...
  const handleGenerateQuote = async (postId: string) => {
    ...
    try {
      const response = await fetch(`/api/posts/${postId}/generate-quote-graphic`, {
        method: 'POST'
      });
      if (response.ok) {
        await fetchPlan();

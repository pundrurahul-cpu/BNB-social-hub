...
export function StrategyDashboard() {
  ...
  useEffect(() => {
    ...
    const loadStrategy = async () => {
      const res = await fetch(`http://103.155.85.64:5001/api/automation/settings/${activeClient.id}`);
      if (res.ok) {
  ...
  const handleTriggerBrain = async () => {
    ...
    try {
      const now = new Date();
      const res = await fetch('http://103.155.85.64:5001/api/automation/smart-strategy', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({

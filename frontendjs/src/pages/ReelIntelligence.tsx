...
export function ReelIntelligence() {
  ...
  const fetchReels = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://103.155.85.64:5001/api/reels?client_id=${activeClient.id}`);
      if (response.ok) {
  ...
  const handleAddReel = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('http://103.155.85.64:5001/api/reels', {
        method: 'POST',
  ...
  const handleAnalyze = async (id: string) => {
    setAnalyzingId(id);
    try {
      const response = await fetch(`http://103.155.85.64:5001/api/reels/${id}/analyze`, {
        method: 'POST'
      });
      if (response.ok) {
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await fetch(`http://103.155.85.64:5001/api/reels/${id}`, {
        method: 'DELETE'
      });
      if (response.ok) {

...
export function Settings() {
  ...
  const handleSaveStrategy = async () => {
    ...
    try {
      ...
      const response = await fetch('http://103.155.85.64:5001/api/automation/save', {
        method: 'POST',
  ...
  const handleTriggerAutomation = async () => {
    ...
    try {
      const now = new Date();
      const response = await fetch('http://103.155.85.64:5001/api/automation/plan-month', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
                ...
                <button
                  onClick={async () => {
                    ...
                    if (activeClient) {
                      try {
                        const { platforms, ...strategyData } = strategy;
                        await fetch('http://103.155.85.64:5001/api/automation/save', {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({
                            client_id: activeClient.id,
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            ...
            <div className="p-6 flex items-center justify-between">
              ...
              <button
                onClick={() => window.location.href = `http://103.155.85.64:5001/api/auth/facebook?client_id=${activeClient?.id || ''}`}
            ...
            <div className="p-6 flex items-center justify-between">
              ...
              <button
                onClick={() => window.location.href = `http://103.155.85.64:5001/api/auth/linkedin?client_id=${activeClient?.id || ''}`}
                className={`px-4 py-2 rounded-lg font-bold text-sm transition-all ${
                  liConnections.length > 0
                    ? 'bg-slate-100 text-slate-600 hover:bg-slate-200'

...
export function Analytics() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [growthRes, industryRes] = await Promise.all([
          fetch(`http://103.155.85.64:5001/api/analytics/growth-history?client_id=${activeClient.id}`),
          fetch(`http://103.155.85.64:5001/api/analytics/industries`)
        ]);
  ...
  const handleFetchPerformance = async (postId: string) => {
    ...
    try {
      const response = await fetch(`http://103.155.85.64:5001/api/analytics/post-performance/${postId}`);
      if (response.ok) {
  ...
  const handleAutoReplyAll = async (postId: string) => {
    ...
    setIsReplying(true);
    try {
      const response = await fetch(`http://103.155.85.64:5001/api/analytics/auto-reply-all/${postId}`, {
        method: 'POST'
      });
      ...
      if (contentType && contentType.indexOf("application/json") !== -1) {
        ...
        if (response.ok) {
          ...
          const commentsRes = await fetch(`http://103.155.85.64:5001/api/analytics/comments/${postId}`);
  ...
  React.useEffect(() => {
    const fetchTrends = async () => {
      ...
      try {
        let url = `http://103.155.85.64:5001/api/analytics/viral-trends/${selectedIndustry}?client_id=${activeClient.id}`;

        if (discoveryMode === 'regional' && region.state && region.district) {
          url = `http://103.155.85.64:5001/api/analytics/viral-trends/regional?country=${region.country}&state=${region.state}&district=${region.district}&industry=${selectedIndustry}&client_id=${activeClient.id}`;
  ...
  const handleExportReport = async () => {
    ...
    setExporting(true);
    try {
      const response = await fetch(`http://103.155.85.64:5001/api/analytics/export?client_id=${activeClient.id}`);
  ...
  const handleRunAnalysis = async (postId: string) => {
    ...
    try {
      // 1. Trigger Analysis
      const response = await fetch(`http://103.155.85.64:5001/api/analytics/analyze-post/${postId}`, {
        method: 'POST',
  ...
  const handleGlobalAnalysis = async () => {
    ...
    setSelectedReport(null);
    try {
      const response = await fetch('http://103.155.85.64:5001/api/analytics/global-analysis', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },

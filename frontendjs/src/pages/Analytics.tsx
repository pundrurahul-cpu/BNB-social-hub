...
export function Analytics() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [growthRes, industryRes] = await Promise.all([
          fetch(`/api/analytics/growth-history?client_id=${activeClient.id}`),
          fetch(`/api/analytics/industries`)
        ]);
  ...
  const handleFetchPerformance = async (postId: string) => {
    ...
    try {
      const response = await fetch(`/api/analytics/post-performance/${postId}`);
      if (response.ok) {
  ...
  const handleAutoReplyAll = async (postId: string) => {
    ...
    setIsReplying(true);
    try {
      const response = await fetch(`/api/analytics/auto-reply-all/${postId}`, {
        method: 'POST'
      });
      ...
      if (contentType && contentType.indexOf("application/json") !== -1) {
        ...
        if (response.ok) {
          ...
          const commentsRes = await fetch(`/api/analytics/comments/${postId}`);
  ...
  React.useEffect(() => {
    const fetchTrends = async () => {
      ...
      try {
        let url = `/api/analytics/viral-trends/${selectedIndustry}?client_id=${activeClient.id}`;

        if (discoveryMode === 'regional' && region.state && region.district) {
          url = `/api/analytics/viral-trends/regional?country=${region.country}&state=${region.state}&district=${region.district}&industry=${selectedIndustry}&client_id=${activeClient.id}`;
  ...
  const handleExportReport = async () => {
    ...
    setExporting(true);
    try {
      const response = await fetch(`/api/analytics/export?client_id=${activeClient.id}`);
  ...
  const handleRunAnalysis = async (postId: string) => {
    ...
    try {
      // 1. Trigger Analysis
      const response = await fetch(`/api/analytics/analyze-post/${postId}`, {
        method: 'POST',
  ...
  const handleGlobalAnalysis = async () => {
    ...
    setSelectedReport(null);
    try {
      const response = await fetch('/api/analytics/global-analysis', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },

...
export function Composer({ editingPostId }: ComposerProps) {
  ...
  useEffect(() => {
    const fetchConnections = async () => {
      try {
        const response = await fetch('http://103.155.85.64:5001/api/auth/connections');
        if (response.ok) {
  ...
  const handleAiRefinement = async () => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        response = await fetch('http://103.155.85.64:5001/api/ai/enhance', { method: 'POST', body: formData });
      } else {
        response = await fetch('http://103.155.85.64:5001/api/ai/enhance', {
          method: 'POST',

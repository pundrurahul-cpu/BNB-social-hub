...
export function Team() {
  ...
  const handleConnect = (platform: string, clientId: string) => {
    ...
    window.location.href = `http://103.155.85.64:5001/api/auth/${platform}?client_id=${clientId}`;
  };
  ...
  const handleAddClient = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('http://103.155.85.64:5001/api/clients', {
        method: 'POST',
  ...
  const handleDeleteClient = async (id: string, name: string) => {
    ...
    try {
      const response = await fetch(`http://103.155.85.64:5001/api/clients/${id}`, {
        method: 'DELETE'
      });
